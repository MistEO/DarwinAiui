...

{
    ...
    if (sr->session_id) {
        ...
        if (sr->notif)
            ...
            sr->notif->_on_speech_end(errcode);
...

{
    ...
    while (sr->rec_stat != MSP_REC_STATUS_COMPLETE) {
        ...
        if (rslt && sr->notif)
            ...
            sr->notif->_on_speech_result(rslt, sr->rec_stat == MSP_REC_STATUS_COMPLETE ? 1 : 0);
    ...
    if (sr->session_id) {
        ...
        if (sr->notif)
            ...
            sr->notif->_on_speech_end(END_REASON_VAD_DETECT);
...
int sr_init(struct speech_rec* sr, const char* session_begin_params,
    ...
    enum sr_audsrc aud_src, AiuiHelper* notify)
...

{
    ...
    session_id = QISRSessionBegin(NULL, sr->session_begin_params, &errcode); //锟斤拷写锟斤拷锟斤拷要锟斤法锟斤拷锟斤拷一锟斤拷锟斤拷锟斤拷为NULL
...

{
    ...
    // std::cout << "in sr_stop_listening, before qisaw, ep_stat=" << sr->ep_stat << std::endl;
    ret = QISRAudioWrite(sr->session_id, NULL, 0, MSP_AUDIO_SAMPLE_LAST, &sr->ep_stat, &sr->rec_stat);
    // std::cout << "in sr_stop_listening, after qisaw, ep_stat=" << sr->ep_stat << std::endl;
...

{
    ...
    // std::cout << "in sr_write_audio_data, before qisaw, ep_stat=" << sr->ep_stat << std::endl;
    ret = QISRAudioWrite(sr->session_id, data, len, sr->audio_status, &sr->ep_stat, &sr->rec_stat);
    // std::cout << "in sr_write_audio_data, after qisaw, ep_stat=" << sr->ep_stat << std::endl;
    ...
    if (MSP_REC_STATUS_SUCCESS == sr->rec_stat) { //锟窖撅拷锟叫诧拷锟斤拷锟斤拷写锟斤拷锟�
